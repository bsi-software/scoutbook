...
For this, select the \node{Desktop} under the orange client node in the Scout Explorer.
...
The map picture returned by the Google API is then read using \java{IOUtility.getContent} and directly fed into the image fields \texttt{setImage} method. 
...
\lstinputlisting[
  ...
  caption=The implementation of the ''Show Map \dots'' link button. %In method \texttt{execClickAction} a new map form is first created, the the address variable is set with the content of the person's location and the form is opened with \texttt{startModify}., 
...
\lstinputlisting[
  label=\lstlabel{mycontacts.server.linkedin.gettoken},
  caption=The necessary access token is created in method \java{getToken} based on the information stored in the database for the logged in user.,
  linerange={168-195},
  float
]
{../code/oneDayTutorial/org.eclipsescout.contacts.server/src/org/eclipsescout/contacts/server/services/LinkedInService.java}
...
For this, a signed OAuth request is first created using the access token provided by method \java{getToken} according to \lstref{mycontacts.server.linkedin.gettoken}. 
