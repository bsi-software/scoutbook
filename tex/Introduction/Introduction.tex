...
Most central of any UI compoment implementations is the UI model component. 
Then, three rendering components for each UI technology supported by Scout. 
...
java/eclipse based: profit from large pool of java developers, low trainnig costs to get to know scout, ramping up new developers is easy
open source: community backup, no vendor lock-in, 
scout hosted at eclipse foundation keeps down risks 
Completely based on Java/Eclipse, Scout Applications are easy to integrate in most IT environments.

Scout excels for implementing enterprise applications.
Specifically if they have to provide a good user experience, need to integrate well with existing infrastructure, and have to be cost effective during implementation and maintenance.

This section conentrates on the user experience and the integration with existing infrastructure.
How scout helps to drive down cost and risks is discussed in section \secref{why_scout}.

scout apps in the enterprise:
...
% --------------------------------------------------------------------------- %
\section{Is Scout production ready?} 

needs text

* 10 years history of being used in production
* technology transitions
* current status: significant number of product installations based on scout, some with large number of users (> 3'000)
* proven performance in several industries: banking, insurance, transportation, government, retail
