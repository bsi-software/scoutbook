...
In general, any set of named entities that remains more or less fixed over time can be seen as a code type. 
They can be used to model the organisational structure of companies, to represent business units or to categorise or segment entities. 
...
public class LookupCall<KEY\_TYPE> implements ILookupCall<KEY\_TYPE>, Cloneable, Serializable, ITypeWithClassId {
...
And the ''search-as-you-type创 functionality of smart fields offers a good usability even if the list contains a large number of elements to choose from. 
...
Pressing the \key{Up Arrow} or the \key{Down Arrow} shows the list of entries when the smart field has the focus.
...
In the example section of the demo form, some of the code types are backed by code types and others by lookup calls. 
