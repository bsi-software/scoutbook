...
Expanding this \element{DesktopService} node, the \java{load} method becomes visible as shown in \figref{helloworld_messagefield}.
...
As the final step of this ''Hello World'' tutorial we deploy the two WAR files that we created in the last section to a Tomcat web server.
...
More on workspaces
The term \textit{workspace} is used for a directory containing the relevant project artefacts such as the Java souce files or configuration files. 
as described in \secref{download_install}.


More on scout project names

More on project alias

One level below the project node the orange client node \node{org.eclipse.scout.helloworld.client}, the green shared node \node{org.eclipse.scout.helloworld.client} and the blue server node  \node{org.eclipse.scout.helloworld.server} are displayed.
The client node holds all application elements that are relevant for the client part of the application only. 
First, the white user interface (UI) technology nodes are shown.
One for each UI technology supported by the Scout application a UI technology node is available. 

more about each node.
...
More on Product launchers

More on products files
