...
The Scout framework uses code types to represent enumerated types\footnote{
Enumerated type: \url{http://en.wikipedia.org/wiki/Enumerated_type}
}. 
And the individual named values of a code type are called codes in Scout. 
The code type and all of its individual codes have associated translated texts, and an id of a specific type. 
Additional features that enhance Scout code types over simple Java enumerations include the possibility to organize code types hierarchically and to dynamically add codes from external sources. 
...
This hierarchical setup can easily be transferred to a hierarchical Scout code type. 
...
The corresponding code for the ICB code type is provided in \lstref{codetype.hierarchical}. 
derived from \java{AbstractCodeType}, parameterized by the type of the id of the code type \java{Long} and the type of the individual codes \java{Long} again
according to convention code type starts with as an ID smallest number of codes, namely 0L, codes increasing according to icb definition
hierachical defined in \java{getConfiguredIsHierarchy}. 

\lstinputlisting[
  label=\lstlabel{codetype.hierarchicalcode},
  caption=An individual code class of the Industry Classification Benchmark.,
...
As an example \lstref{codetype.hierarchicalcode}.
derived from \java{AbstractCode}, parameterized by the type individual codes \java{Long} again
ID according to icb definition
