...
products\footnote{Product files define all necessary elements constituting an application in an XML document.}.
...
For this, either click on the icon with the white triangle in the green circle of the right hand side of the box, or, click on the green bug right below to start the server in debug mode.
...
This method has been added in the project creation step by the Scout SDK.
It is called by the Scout framework after the desktop frame of the client application is created and becomes visible on the display.
...
We now navigate to the class \java{DesktopForm} which is located in the \folder{Forms} below the Desktop node in the Scout Explorer.
