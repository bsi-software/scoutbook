...
To find the correct place to add the field, we first navigate to the orange \node{client} in the Scout Explorer and double click the \node{Desktop} just below.
This will open the associated Java file \texttt{Desktop.java} in the editor view of the Scout SDK.

The desktop represents the container of all visible elements in the Scout client.
Of interest at this stage is the overwritten callback method \java{execOpend} shown in \lstref{helloworld.execopend}.
This method has been added in the project creation step by the Scout SDK described in \secref{create_project_simple}.
...
The only thing that happens here is the creation of the 
form\footnote{A Scout form is a UI container that can either be displayed in its own window or shown as a view inside of another UI container.}
\java{desktopForm}.
The desktop form then gets assigned an icon before it is started via method \java{startView}.
The class \java{DesktopForm} has also been added in the initial project creation step.
In the next paragraph we will describe how to navigate to the class \java{DesktopForm} and what method \java{startView} is doing.
...
In the Explorer View of the Scout SDK expand the orange \node{client}
\footnote{To expand elements (nodes, folders, etc.) in the Scout Explorer of the Scout SDK use a double click on the element or a single click on the plus icon in front of the element.}.
Below it, you will find the \folder{Forms}. 
Expanding the Forms folder will then show the \element{DesktopForm} as shown in \figref{helloworld_viewhandler}.
According to this screenshot, drill down to the \element{ViewHandler} element.
A double on this ViewHandler element opens the corresponding source code in the Java editor.
The corresponding code is also shown in \lstref{helloworld.viewhandler}.

We will now have a more detailed look at the view handler of class \java{DesktopForm}.
The class \java{ViewHandler} is responsible for the live cycle handling of our desktop form.
In the Scout framework form handler classes are used to specify custom behaviour before a form is opend, after it is closed, or when entered data needs to be saved. 
Overwriting the correct callback method of a form handler is enough.
The necessary wiring is provided by the framework. 
According to \lstref{helloworld.viewhandler} only the \java{execLoad} method is implemented in the handler of the desktop form. 
Before we discuss the provided implementation of method \java{execLoad}, we want to shed some light on when this method will actually be called.
As we have seen in the \java{Desktop} class in \lstref{helloworld.execopend}, the form's method \java{startView} is called after creating the desktop form.
...
This eventually leads to the call of our custom implementation of \java{execLoad}.

We are now ready to describe the implementation of method \java{execLoad} of the desktop form's view handler.
The central piece is the call of the \java{load} method emphasized in \lstref{helloworld.viewhandler}.
In the code before, a reference to a process service is first obtained.
...
The code of \java{ViewHandler} is provided in \lstref{helloworld.viewhandler} in the context of \java{DesktopForm} and its \java{startView} method.
