...
In this section we export the finished ''Hello World'' application as WAR files using the Scout SDK.
In the context of this book WAR files\footnote{
Web application ARchive (WAR): \url{http://en.wikipedia.org/wiki/WAR_file_format_(Sun)}
} 
represent complete Scout applications that can be deployed individually to a web server.
For the full setup there will be two WAR files.
The first WAR file contains the Scout server application including a zipped desktop client application ready for downloading.
The second WAR file contains the RAP Server application needed for the Scout web and mobile client applications.
...
To start the export wizard, we need to have the Scout SDK started with the ''Hello World'' Scout project we want to export.
In the Scout Explorer we then select corresponding context menu on the ''Hello World'' top level application node as shown in \figref{sdk_export_war}.
...
\begin{figure}
\includegraphics[width=13cm]{export_wizard_1.png}
...
In the last dialog of the \wizard{Export Scout Project} shown in \figref{export_wizard_4}, the RAP server product to be exported and the WAR file name are specified.
Normally, the proposed field values are all we need and we can close the wizard with \button{Finish}.
After this last step, the Scout SDK is building all necessary artefacts for the ''Hello World'' application and building the two WAR files according to the settings made in the wizard.
For deploying the ''Hello World'' application to the Tomcat webserver these two WAR files are the only items needed.


% --------------------------------------------------------------------------- %
\section{Deploying to Tomcat}

As the final step of this ''Hello World'' tutorial we will deploy the WAR files we created in the last section to a Tomcat webserver.
Before we can deploy any WAR files we need a working Tomcat installation.
For this, you may want to read and follow the instructions provided in \appref{install_tomcat}.
To verify a running Tomcat instance type the URL \url{http://localhost:8080/} into the address bar of the web browser of your choice.
...
Once the web browser shows the successful running of your Tomcat instance, switch to its ''Manager App'' by clicking on the button highlighted in \figref{deploy_tomcat_1}.
...
If you don't know the correct username or password you may look it up in the file \filename{tomcat-users.xml} according to \lstref{tomcat.users}.
This file is located in subdirectory \filename{conf} of your Tomcat installation.
...
\caption{TODO: Bad Server URL!!! The ''Hello World'' home page, providing a link to download the desktop client.}
...
\figref{helloworld_running_clients} shows screenshots for a desktop browser and a mobile browser.
