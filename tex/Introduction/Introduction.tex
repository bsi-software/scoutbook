...
In this section we will just two common pitfalls when working with the Scout IDE and Tomcat simultaneously.
Both can can prevent a web application from running in Tomcat or properly updating.

In the first case the Jetty web server running inside the Scout SDK and Tomcat can block the 8080 port once they are running.
...
To avoid such conflicts make sure that you always stop the Scout server application in the Scout SDK before you restart your Tomcat server.
Alternatively you can assign two different ports to your Jetty webserver and your Tomcat webserver.
To modify the port number used in the Scout SDK you have to update the corresponding properties in the \filename{config.ini} files of the development products of your Scout server application and all client applications.
...
\begin{tabular}{ll}
 ...
 Scout SWT/Swing Client & server.url=http://localhost:8081/helloworld_server/process \\
 Scout RAP Server       & server.url=http://localhost:8081/helloworld_server/ajax
\end{tabular}

The second pitfall is connected to a web application that seems to refuse to update to a fixed and freshly generated WAR file.
...
In case you are interested in more advances aspects of Apache Tomcat you may want to have a look at the article of Chua Hock-Chuan\footnote{
