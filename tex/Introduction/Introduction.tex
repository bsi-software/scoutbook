...
The simplest option to move a Scout application from the development environment into the 'wild' is to use the \wizard{Export Scout Project} provided by the Scout SDK.
Using standard settings, this wizard allows to create two WAR files that represent a complete application.
The first WAR file holds the Scout server including a landing page to download the Scout desktop client in a standalone ZIP file.
In the second WAR file the Ajax server based on Eclipse RAP is contained.

\begin{figure}
\includegraphics[heigth=5cm]{helloworld_server_war.png} \hspace{5mm}
\includegraphics[heigth=5cm]{helloworld_server_war_eclipse.png}
...
The organisation of the exported WAR files is defined according to server-side Equinox, the typical setup for running Eclipse based applications on the server-side.
See \appref{osgi_basics}, for further material regarding (server-side) Equinox.
As a concrete example we will first describe the general organisation of the \filename{helloworld_server.war} Scout server WAR file.
Then, we can introduce specific artefacts of interest that are contained in this WAR file.
See \figref{helloworld_server_war} for the directory tree of the \filename{helloworld_server.war} file.

\begin{figure}
\includegraphics[heigth=9cm]{helloworld_server_war_plugin.png} 
\caption{The content of the ''Hello World'' server plugin contained in the \java{helloworld_server.war} file.
...
\begin{figure}
\includegraphics[heigth=8cm]{helloworld_server_plugin.png} 
