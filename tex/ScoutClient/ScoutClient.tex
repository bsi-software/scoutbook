...
Such as fields to handle and display list, tree and table data. 
As in the previous chapter, for each described form field example use cases, screen shots and code snippets are provided. 

% --------------------------------------------------------------------------- %
\section{List Box}

List boxes allow a user to select of a subset of the presented elements. 
The individual elements are all displayed in the form of checkable options and are provided to the list box in the form of a code type or a lookup call. 
...
\lstinputlisting[
  ...
  caption=A simple ListBox field backed with a code type representing different colors.,
  index={ListBox},
  linerange={172-189},
  float
]
...
A number of use cases are provided in the ''ListBox´´ form shown in \figref{listbox}. 
These list box examples use either code types or lookup calls as their data source. 
A simple code example for a list box backed by a code type is provided in \lstref{field.listbox}. 

In the \field{List Content} located on the right hand side of the configuration box, it is also possible to enter user defined content. 
...
Tree boxes are working very similar to list boxes. 
The only major difference to the list box is the fact that the elements presented are represented in the form of trees. 
