...
In the first step we use the \wizard{Export Scout Project} to export the ''Hello World'' client server application from the Scout SDK into two WAR files\footnote{
Web application ARchive (WAR): \url{http://en.wikipedia.org/wiki/WAR_file_format_(Sun)}
}.
To start the export wizard select the top level node for the ''Hello World'' application in the Scout Explorer as shown in \figref{sdk_export_war}.
In the first wizard step, shown in \figref{export_wizard_1} the target directory for the WAR files needs to be specified.
...
\begin{figure}
\includegraphics[width=10cm]{sdk_export_war_menu.png} 
...
\begin{figure}
\includegraphics[width=10cm]{export_wizard_4.png}
...
Before we can deploy the ''Hello World'' WAR files we need a working Tomcat installation.
